...
import "../styles/ludoscorem.scss";

const LudoScoreM= ({ user, handleClose }) => {
  ...
  // useEffect(() => {
  //   const savedUser = JSON.parse(localStorage.getItem("updatedUser"));
    
  //   if (savedUser) {
  //     setUpdatedUser(savedUser); // Set the updated data from localStorage
  //   } else if (user) {

  //     setUpdatedUser(user); // Fallback to initial user prop if no updated data in localStorage
  //   }
   
  //   setLoading(false); // Set loading to false when data is ready
  // }, [user]); // Depend on the user prop, so it will re-run when user changes
  ...
  const handleUpdate = async (updatedUser) => {
    try {
      ...
      const response = await axios.patch(`http://localhost:3001/ludoscore/ludoscorectg/${updatedUser._id}`, updatedUser);
